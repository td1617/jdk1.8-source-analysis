...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * shrinkage.
     */
    static final int TREEIFY_THRESHOLD = 8;//将链表转换成红黑树的阈值
...
//    除此之前，还有就是代码的第38行处有个++size，我们这样想，还是线程A、B，这两个线程同时进行put操作时，假设当前HashMap的zise大小为10，当线程A执行到第38行代码时，从主内存中获得size的值为10后准备进行+1操作，但是由于时间片耗尽只好让出CPU，线程B快乐的拿到CPU还是从主内存中拿到size的值10进行+1操作，完成了put操作并将size=11写回主内存，然后线程A再次拿到CPU并继续执行(此时size的值仍为10)，当执行完put操作后，还是将size=11写回内存，此时，线程A、B都执行了一次put操作，但是size的值只增加了1，所有说还是由于数据覆盖又导致了线程不安全。
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)//第1步，判断当前桶是否为空，空的话就需要初始化 ，resize（）中判断是否进行初始化
            n = (tab = resize()).length;
        if ((p = tab[i = (n - 1) & hash]) == null)//第2步，根据当前key的hashcode定位到具体的桶中并判断是否为空，为空表明没有 Hash 冲突就直接在当前位置创建一个新桶即可。
            tab[i] = newNode(hash, key, value, null);
        else {//定位到的桶的位置不是空
            Node<K,V> e; K k;
            if (p.hash == hash &&
                ((k = p.key) == key || (key != null && key.equals(k))))//第3步，如果当前桶有值（ Hash 冲突），那么就要比较当前桶中的 key、key 的 hashcode 与写入的 key 是否相等，相等就赋值给 e,在第 8 步的时候会统一进行赋值及返回。
                e = p;
            else if (p instanceof TreeNode)//第4步，如果当前桶为红黑树，那就要按照红黑树的方式写入数据。
                e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
            else {
                //链表
                for (int binCount = 0; ; ++binCount) {
                    //第5步，如果是个链表，就需要将当前的 key、value 封装成一个新节点写入到当前桶的后面（形成链表）。
                    if ((e = p.next) == null) {
                        p.next = newNode(hash, key, value, null);
                        if (binCount >= TREEIFY_THRESHOLD - 1)
                            //第6步，判断当前链表的大小是否大于预设的阈值，大于时就要转换为红黑树。
                            treeifyBin(tab, hash);
                        break;
                    }
                    //第7步，如果在遍历过程中找到 key 相同时直接退出遍历。
                    if (e.hash == hash &&
                        ((k = e.key) == key || (key != null && key.equals(k))))
                        break;
                    p = e;
            ...
            //第8步，如果 e != null 就相当于存在相同的 key,那就需要将值覆盖。
            if (e != null) {
                V oldValue = e.value;
        ...
        ++modCount;
        if (++size > threshold)//第9步，判断是否需要进行扩容。
            resize();
    ...
    final Node<K,V>[] resize() {
        ...
        int oldCap = (oldTab == null) ? 0 : oldTab.length;//原数组如果为null，则长度赋为0
    ...
    final void treeifyBin(Node<K,V>[] tab, int hash) {
        ...
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)//这里还有一个限制条件，当table的长度小于MIN_TREEIFY_CAPACITY（64）时，只是进行扩容
            resize();
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        final void treeify(Node<K,V>[] tab) {
            ...
            for (TreeNode<K,V> x = this, next; x != null; x = next) {//遍历链表中的每一个树节点，当前节点是x
