...
public abstract class AbstractQueuedSynchronizer
     ...
     */
    //阻塞的尾节点，尾插法，形成一个隐视的链表
    private transient volatile Node tail;

    /**
     * The synchronization state.
     */
    //代表当前锁的状态，0代表没被占用，>0代表有线程持有当前锁（之所以>0而不是=1，是因为锁可以重入，重入后state会加1）
     ...
     */
    //采用自旋的方式入队
    //到这边代码只有2种可能:1、等待队列为空，2、CAS失败(有线程在竞争入队)
    //自旋在这边的语义是，CAS在设置tail的过程中，竞争一次不行我就竞争多次
    private Node enq(final Node node) {
        for (;;) {
            Node t = tail;
            //队列是空的，创建一个空的标志结点作为head结点，并将tail也指向它。
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        // 简单说，如果前驱节点取消了排队，
        // 找前驱节点的前驱节点，往前循环总能找到一个waitStatus<=0的节点
        if (ws > 0) {
            /*
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            //又是自旋
            for (;;) {
                //获得node节点的prev节点（node的上一个节点）前驱节点
                ...
                // 到这里，说明上面的if分支没有成功，要么当前node本来就不是队头，要么就是tryAcquire(arg)没有抢赢别人，继续往下看
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&//先tryAcquire(1)，如果tryAcquire没有成功,把当前线程挂起，放入阻塞队列中
    ...
    public final boolean releaseShared(int arg) {
        //只有当state减为0的时候，tryReleaseShared才返回true
     ...
     */
    //将节点从条件队列转移到阻塞队列
    //true代表成功转移，false代表在signal之前，节点就已经取消了
    final boolean transferForSignal(Node node) {
        /*
         * If cannot change waitStatus, the node has been cancelled.
         */
        //CAS失败说明此 node 的 waitStatus 已不是 Node.CONDITION，说明节点已经取消，
        ...
        if (ws > 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL))
            // 如果前驱节点取消等待或者 CAS 失败，会进到这里唤醒线程，之后的操作看下一节
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         * storms.
         */
        //等待队列是一个单项链表，遍历链表将已经取消等待的节点清除出去
        ...
        public final void await() throws InterruptedException {
            ...
            int interruptMode = 0;
            //如果当前节点不在同步队列上，即还没有被signal，则将当前线程阻塞
            while (!isOnSyncQueue(node)) {
                //线程挂起
                LockSupport.park(this);
            ...
            //退出自旋说明当前节点已经在同步队列上，但是当前节点不一定在同步队列队首，acquireQueued将阻塞直到当前节点成为队首，即当前线程获得了锁。然后await()方法就可以退出了，让线程继续执行await()后的代码。
